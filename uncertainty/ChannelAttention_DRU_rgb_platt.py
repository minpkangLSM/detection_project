...
class CA_DeepResUNet :
    ...
    def pretrained(self):
        ...
        E_channel = Conv2D(filters=E_channel.shape[3],
                           ...
                           kernel_size=3,
                           kernel_initializer="he_normal",
                           name="conv_e_channel_1",
                           trainable=False)(E_channel)
        E_channel = BatchNormalization(trainable=False)(E_channel)
        E_channel = Activation(activation='relu')(E_channel)
        E_channel = Conv2D(filters=E_channel.shape[3],
                           padding="same",
                           kernel_size=3,
                           kernel_initializer="he_normal",
                           name="conv_e_channel_2",
                           trainable=False)(E_channel)
...
if __name__ == "__main__":
    ...
    # Pretrained model
    # pretrained = CA_DeepResUNet(input_shape=INPUT_SHAPE, lr=LR, num_classes=NUM_CLASSES,
    #                             batch_size=BATCH_SIZE).pretrained()
    # pretrained.load_weights("./DRU_CA_rgb_callbacks/finetune_weights/DRU_CA_rgb_49.hdf5")

    # Calibration model
    CA_DRU = CA_DeepResUNet(input_shape=INPUT_SHAPE, lr=LR, num_classes=NUM_CLASSES, batch_size=BATCH_SIZE)
    model = CA_DRU.build_net()
    # for i in range(len(pretrained.layers) - 1):
    #     model.layers[i].set_weights(pretrained.layers[i].get_weights())
    model.load_weights("./DRU_CA_rgb_callbacks/calibrated_weights/DRU_CA_rgb_calib_25.hdf5")
    model.summary()
    ...
    # callbacks = [
    #     tf.keras.callbacks.ModelCheckpoint(filepath="./DRU_CA_rgb_callbacks/calibrated_weights/DRU_CA_rgb_calib_{epoch:02d}.hdf5"),
