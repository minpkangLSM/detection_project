...
class DeepResUNet :
    ...
    CODE INFO
    """
    def __init__(self, input_shape, offground_shape, ground_shape, lr, num_classes, weight_decay):
...
if __name__ == "__main__" :
    ...
    pred_ground_generator = pred_img_datagen.flow_from_directory(
        directory="./renewal_dataset_0324/valid/ground",
        target_size=(512, 512),
        batch_size=1,
        shuffle=False,
    ...
    pred_offground_generator = pred_img_datagen.flow_from_directory(
        directory="./renewal_dataset_0324/valid/offground",
        target_size=(512, 512),
        batch_size=1,
        shuffle=False,
    ...
    # Target model
    DeepResUNet = DeepResUNet(input_shape=INPUT_SHAPE, offground_shape=OFFGROUND_SHAPE, ground_shape=GROUND_SHAPE, lr=LR, num_classes=NUM_CLASSES, weight_decay=weight_decay)
    model = DeepResUNet.build_net()

    for i in range(4, len(pretrained.layers)):
        model.layers[i+4].set_weights(pretrained.layers[i].get_weights())

    model.load_weights("./DRU_rgbd_weights_float(finetune_b_on_rgb)/DRU_48.hdf5")
    ...
    # callbacks = [
    #     tf.keras.callbacks.ModelCheckpoint(filepath="./DRU_rgbd_weights_float(finetune_b_on_rgb)/DRU_{epoch:02d}.hdf5"),
    #     tf.keras.callbacks.TensorBoard(log_dir="./DRU_rgbd_logs_float(finetune_b_on_rgb)", update_freq="batch"),
    #     # VISUALIZE
