...
def class2color(mask, name):
    ...
    tifi.imwrite("./data_for_sangam_pred/valid/SA_DRU/"+ name+".tif", mask[:, :, 1])
...
if __name__ == "__main__":
    ...
    SA_DRU = SA_DeepResUNet(input_shape=INPUT_SHAPE, lr=LR, num_classes=NUM_CLASSES)
    model = SA_DRU.build_net()
    model.load_weights("./DRU_SA_rgb_callbacks/finetune_weights/DRU_SA_rgb_49.hdf5")
    model.summary()
    ...
    # callbacks = [
    #     tf.keras.callbacks.ModelCheckpoint(filepath="./DRU_SA_rgb_callbacks/finetune_weights/DRU_SA_rgb_{epoch:02d}.hdf5"),
